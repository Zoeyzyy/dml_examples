...
if args.from_to == "from":
    sum_path = './Sum_by_time/Sum_by_Time_from108_' + str(10 ** -(accuracy - 9.0))
else:
    sum_path = './Sum_by_time/Sum_by_Time_to108_' + str(10 ** -(accuracy - 9.0))
    
step_path = './step2time_index/step2time_index_108_' + str(accuracy)

if (not os.path.exists(sum_path)) or (not os.path.exists(step_path)):
    # 定义要搜索的文件夹路径
    folder_path = "./packet/"
...
if hasattr(args, 'epoch_num') and args.epoch_num is not None:
    ...
    plt.figure(figsize=(40, 4), dpi=1200)
